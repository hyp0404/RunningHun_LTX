...
# Used only by workflows that expose a real negative-prompt text field. The
# current LTX 2.3 PromptRelay workflow creates negative conditioning inside
# LTXVConditioning and therefore has no standalone negative text node.
...
# PromptRelay-only workflows cannot accept a separate negative prompt. Keep a
...
def filter_workflow_node_map(
    workflow: dict[str, Any], node_map: dict[str, NodeTarget]
) -> tuple[dict[str, NodeTarget], dict[str, NodeTarget]]:
    """Split configured mappings into fields that exist and stale mappings.

    Railway may retain an older ``LTX23_NODE_MAP_JSON`` after the exported
    workflow changes. Optional stale roles, notably the former ``4.text``
    negative prompt, must not make inspection fail or leak into nodeInfoList.
    Required roles are still rejected by the normal required-role validation.
    """

    valid: dict[str, NodeTarget] = {}
    invalid: dict[str, NodeTarget] = {}
    for role, target in node_map.items():
        node = workflow.get(target.node_id)
        inputs = node.get("inputs") if isinstance(node, dict) else None
        if isinstance(inputs, dict) and target.field_name in inputs:
            valid[role] = target
        else:
            invalid[role] = target
...
def inspect_ltx_workflow(settings: Settings) -> dict[str, Any]:
    ...
    configured = dict(settings.ltx.node_map)
    resolved, invalid = filter_workflow_node_map(workflow, configured)
    ...
    return {
        ...
        "ignored_optional_mappings": {
            role: target.to_dict()
            for role, target in sorted(invalid.items())
...
async def submit_ltx_for_record(
    ...
    if settings.ltx_mode == "workflow":
        ...
        ltx_map, invalid_workflow_mappings = filter_workflow_node_map(
            workflow_template, dict(settings.ltx.node_map)
        )
        for role, target in sorted(invalid_workflow_mappings.items()):
            logger.warning(
                "Ignoring stale LTX workflow mapping role=%s target=%s.%s",
                role,
                target.node_id,
                target.field_name,
            )
    ...
    if workflow_template is not None:
        ...
        workflow_preflight = {
            ...
            "mandatoryNoTextNegativeApplied": (
                "negative_prompt" in ltx_map
                and MANDATORY_NO_TEXT_NEGATIVE.lower()
                in str(values.get("negative_prompt") or "").lower()
            ),
            "globalNoWritingGuardApplied": (
                GLOBAL_NO_WRITING_GUARD in str(values.get("prompt") or "")
            ),
        }
